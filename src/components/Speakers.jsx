...
const Info = [
  ...
  {
    name: 'Sistema de seguridad y salud en el trabajo',
    date: 'ISO 45001:2018 ',
    Iso: [
      {
        name: 'ISO 45001',
    ...
    >
      ...
      <Container>
        <div className="mx-auto max-w-2xl lg:mx-0">
          <h2
            id="speakers-title"
            className="font-display text-4xl font-medium tracking-tighter text-sky-500 sm:text-5xl"
          >
            Asesorías
          </h2>
          <p className="mt-4 font-display text-2xl tracking-tight text-gray-900">
            section to explain whatever we need.
          </p>
        </div>
        <Tab.Group
          as="div"
            ...
            <Tab.List className="grid auto-cols-auto grid-flow-col justify-start gap-x-8 gap-y-10 whitespace-nowrap px-4 sm:mx-auto sm:max-w-2xl sm:grid-cols-3 sm:px-0 sm:text-center lg:grid-flow-row lg:grid-cols-1 lg:text-left">
              {({ selectedIndex }) =>
                Info.map((info, infoIndex) => (
                  <div key={info.dateTime} className="relative lg:pl-8">
                    <DiamondIcon
                      className={clsx(
          ...
          <Tab.Panels className="lg:col-span-3">
            {Info.map((info) => (
              <Tab.Panel
                ...
                unmount={false}
              >
                {info.Iso.map((iso, isoIndex) => (
                  <div key={isoIndex} className="md:col-start-2">
                    <div className="group relative h-[17.5rem] transform overflow-hidden rounded-4xl">
                      <div
                        className="absolute inset-0 bg-indigo-50"
                        style={{ clipPath: `url(#${id}-${isoIndex % 3})` }}
                      >
                        <Image
